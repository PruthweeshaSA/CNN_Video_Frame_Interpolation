...
if __name__ == '__main__':
    ...
    # using absolute path
    input_data_dir = Path(pathlib.Path('../data/test/stray.mov')).resolve()

    input_data_dir = str(input_data_dir)

    input_data_dir = input_data_dir.replace('\\', '/')

    input_data_dir = Path(input_data_dir)

    temp_data_dir = Path(pathlib.Path('../tempfiles')).resolve()
    output_data_dir = pathlib.Path('../video')

    width, height, fps_in = generate_frames(input_data_dir, temp_data_dir)
    generate_video(model_dir, temp_data_dir, output_data_dir, fps_in, width, height)
