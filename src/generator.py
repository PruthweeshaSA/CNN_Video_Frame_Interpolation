...
def gen_batch(file_list, total_samples):
    ...
    while True:
        ...
        for i in range(0, BATCH_SIZE):
            X_data[i, :, :, :3] = imageio.imread(
                os.path.join("/content/drive/My Drive/Colab Notebooks/" + str(file_subset[i]), "im1.png"))
            X_data[i, :, :, 3:] = imageio.imread(
                os.path.join("/content/drive/My Drive/Colab Notebooks/" + str(file_subset[i]), "im3.png"))
            Y_data[i, :, :, :] = imageio.imread(
                os.path.join("/content/drive/My Drive/Colab Notebooks/" + str(file_subset[i]), "im2.png"))
